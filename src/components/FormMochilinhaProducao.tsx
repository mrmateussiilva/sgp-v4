...
interface FormMochilinhaProducaoProps {
    tabId: string;
    tabData: any;
    vendedores: string[];
    designers: string[];
...
export function FormMochilinhaProducao({
    tabId,
    tabData,
    vendedores,
    designers,
    ...
    const valorMochilinha = useMemo(() => parseBR(tabData?.valor_mochilinha || '0,00'), [tabData?.valor_mochilinha]);
    ...
    const quantidade = useMemo(() => {
        const quantidadeRaw = parseInt(tabData?.quantidade_mochilinha || '1', 10);
        return Number.isNaN(quantidadeRaw) || quantidadeRaw <= 0 ? 0 : quantidadeRaw;
    }, [tabData?.quantidade_mochilinha]);

    const valorUnitario = valorMochilinha + outrosValores;
        ...
        <div className="space-y-6">
            <div className="flex items-end gap-4">
                <div className="flex-1 space-y-2">
                    <Label className="text-base font-medium">Descrição do produto *</Label>
                    <Input
                        value={tabData?.descricao || ''}
                        onChange={(event) => onDataChange('descricao', event.target.value)}
                        placeholder="Ex: Mochilinha saco promocional 30x40cm"
            ...
            <div className="grid grid-cols-3 gap-4">
                <div className="space-y-2">
                    <Label className="text-base font-medium">Material (Tecido)</Label>
                    <Select
            ...
            <div className="space-y-4">
                <Label className="text-base font-medium">Tipo de acabamento</Label>
                <Select
                    value={tabData?.tipo_acabamento || 'nenhum'}
                    onValueChange={(value) => onDataChange('tipo_acabamento', value)}
                >
                    <SelectTrigger className="bg-white h-12 text-base border-slate-200">
                        <SelectValue placeholder="Selecione o acabamento" />
                    </SelectTrigger>
                    <SelectContent>
                        <SelectItem value="nenhum">Nenhum</SelectItem>
                        <SelectItem value="alca">Alça</SelectItem>
                        <SelectItem value="cordinha">Cordinha</SelectItem>
                        <SelectItem value="alca_cordinha">Alça + Cordinha</SelectItem>
                    </SelectContent>
                </Select>
            </div>
                ...
                <div className="space-y-2 flex flex-col">
                    ...
                    <div className="relative flex-1 min-h-[320px]">
                        <Input
                            ...
                            className="hidden"
                            id={`upload-imagem-produto-${tabId}`}
                        />
                        <label
                            htmlFor={`upload-imagem-produto-${tabId}`}
                            onDragOver={(e) => {
                                e.preventDefault();
                                e.stopPropagation();
                            }}
            ...
            <div className="space-y-4">
                <div className="grid grid-cols-4 gap-4">
                    <div className="space-y-2">
                        ...
                        <CurrencyInput
                            value={tabData?.valor_mochilinha ?? '0,00'}
                            onValueChange={(formatted) => onDataChange('valor_mochilinha', formatted)}
                            placeholder="25,00"
                    ...
                    <div className="space-y-2">
                        ...
                        <Input
                            type="number"
                            min="1"
                            value={tabData?.quantidade_mochilinha || '1'}
                            onChange={(event) => onDataChange('quantidade_mochilinha', event.target.value)}
                            placeholder="1"
                            className="h-12 text-base"
                        />
                    </div>
                ...
                {(valorMochilinha > 0 || outrosValores > 0) && (
                    <div className="p-4 bg-gradient-to-br from-emerald-500 to-green-600 rounded-xl text-white space-y-2">
                        ...
                        <div className="space-y-1 text-sm">
                            <div className="flex justify-between">
                                <span>Valor base do produto:</span>
                                <strong>R$ {formatBR(valorMochilinha)}</strong>
                            </div>
                            {outrosValores > 0 && (
                            ...
                            <div className="flex justify-between text-lg font-bold bg-white bg-opacity-20 px-3 py-2 rounded-lg mt-2">
                                <span>Total ({tabData?.quantidade_mochilinha || 1} unidade(s)):</span>
