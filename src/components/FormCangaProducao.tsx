...
interface FormCangaProducaoProps {
  tabId: string;
  tabData: any;
  vendedores: string[];
  designers: string[];
...
export function FormCangaProducao({
  tabId,
  tabData,
  vendedores,
  designers,
  ...
  const valorCanga = useMemo(() => parseBR(tabData?.valor_canga || '0,00'), [tabData?.valor_canga]);
  ...
  const quantidade = useMemo(() => {
    const quantidadeRaw = parseInt(tabData?.quantidade_canga || '1', 10);
    return Number.isNaN(quantidadeRaw) || quantidadeRaw <= 0 ? 0 : quantidadeRaw;
  }, [tabData?.quantidade_canga]);

  const valorUnitario = valorCanga + outrosValores;
    ...
    <div className="space-y-6">
      <div className="flex items-end gap-4">
        <div className="flex-1 space-y-2">
          <Label className="text-base font-medium">Descrição da canga *</Label>
          <Input
            value={tabData?.descricao || ''}
            onChange={(event) => onDataChange('descricao', event.target.value)}
            placeholder="Ex: Canga promocional 1,5x1m"
      ...
      <div className="space-y-2">
        <div className="flex items-center space-x-2">
          <Checkbox
            id={`baininha-${tabId}`}
            checked={tabData?.baininha || false}
            onCheckedChange={(checked) => onDataChange('baininha', checked)}
          />
          <Label
            htmlFor={`baininha-${tabId}`}
      ...
      <div className="grid grid-cols-2 gap-6 items-stretch">
        <div className="space-y-4">
          <div className="space-y-2">
            ...
            <Textarea
              ...
              placeholder="Detalhes adicionais sobre a canga"
              rows={5}
              className="text-sm"
            />
          </div>
        ...
        <div className="space-y-2 flex flex-col">
          <Label className="text-base font-semibold">Imagem da canga</Label>
          <div className="relative flex-1 min-h-[320px]">
            <Input
              ...
              className="hidden"
              id={`upload-imagem-canga-${tabId}`}
            />
            <label
              htmlFor={`upload-imagem-canga-${tabId}`}
              onDragOver={(e) => {
                e.preventDefault();
                e.stopPropagation();
              }}
      ...
      <div className="space-y-4">
        <div className="grid grid-cols-4 gap-4">
          <div className="space-y-2">
            <Label className="text-base font-medium">Valor da canga (R$)</Label>
            <CurrencyInput
              value={tabData?.valor_canga ?? '0,00'}
              onValueChange={(formatted) => onDataChange('valor_canga', formatted)}
              placeholder="75,00"
          ...
          <div className="space-y-2">
            ...
            <Input
              type="number"
              min="1"
              value={tabData?.quantidade_canga || '1'}
              onChange={(event) => onDataChange('quantidade_canga', event.target.value)}
              placeholder="1"
              className="h-12 text-base"
            />
          </div>
          ...
          <div className="p-4 bg-gradient-to-br from-emerald-500 to-green-600 rounded-xl text-white space-y-2">
            ...
            <div className="space-y-1 text-sm">
              <div className="flex justify-between">
                <span>Valor base da canga:</span>
                <strong>R$ {formatBR(valorCanga)}</strong>
              </div>
              {outrosValores > 0 && (
              ...
              <div className="flex justify-between text-lg font-bold bg-white bg-opacity-20 px-3 py-2 rounded-lg mt-2">
                <span>Total ({tabData?.quantidade_canga || 1} canga(s)):</span>
