...
5. **V2.0 Desacoplamento Global de Modais**: Todo o ecossistema de modais densos (Edição, Deleção, Cópia, Substituição) hospedados ativamente dentro do monolito de tabela principal foi desmembrado em componentes Lazy na pasta `src/components/modals` envelopados num Provedor `OrderModalsProvider.tsx`. A reatividade local agora é pilotada integralmente via *Zustand* (`useModalStore.ts`), arrancando centenas de linhas JSX e funções `useState` invisíveis presas indevidamente na Render Tree principal.
